...
var Main = function() {
	...
	logging.Logging.info("hello",null,null,{ fileName : "Main.hx", lineNumber : 15, className : "Main", methodName : "new"});
};
$hxClasses["Main"] = Main;
Main.__name__ = ["Main"];
...
var interpolation = {}
interpolation.Template = function(template) {
	this.template = template;
};
$hxClasses["interpolation.Template"] = interpolation.Template;
interpolation.Template.__name__ = ["interpolation","Template"];
interpolation.Template.prototype = {
	safe_substitute: function(context) {
		return this._substitute(context,true);
	}
	,substitute: function(context) {
		return this._substitute(context,false);
	}
	,_substitute: function(context,safe) {
		var $char;
		var char_code;
		var char_is_id = false;
		var output = [];
		var variable_name;
		var point_delimiter = -1;
		var point_left_bracket = -1;
		var point_right_bracket = -1;
		var point_variable_start = -1;
		var point_variable_end = -1;
		var size = this.template.length;
		var lineno = 1;
		var col = 1;
		var value;
		var state = 0;
		var state_not_interpolation = 0;
		var state_interpolation = 1;
		var state_matching_variable = 2;
		var state_matched_variable = 3;
		var state_escape_dollar = 4;
		var error_invalid_placeholder = 5;
		var state_last_character = false;
		var i = 0;
		while(i < size) {
			$char = this.template.charAt(i);
			char_code = HxOverrides.cca($char,0);
			...
			if(state == state_not_interpolation) {
				if("$" == $char) {
					...
					this.log("State changed to interpolation");
				}
			} else if(state == state_interpolation) {
				char_is_id = this.is_id($char);
				if("$" == $char) {
					point_delimiter = -1;
					...
					this.log("Matched right bracket and change state to matched variable.");
					...
					if(point_left_bracket > -1) {
						if(point_right_bracket == -1) {
							...
							this.log("error: Missing right bracket.");
						}
						point_variable_end = i;
					}
					if(i == size - 1 && !char_is_id) i -= 1; else if(i == size - 1 && char_is_id) point_variable_end += 1;
					this.log("State matched variable");
				}
			}
			if(state == state_not_interpolation) output.push($char);
			if(state == state_matched_variable) {
				state = state_not_interpolation;
				variable_name = this.template.substring(point_variable_start,point_variable_end);
				this.log("variable variable_name:" + variable_name);
				if(!context.exists(variable_name)) state = error_invalid_placeholder; else {
					value = Std.string(context.get(variable_name));
					output.push(value);
					if(-1 == point_left_bracket && !state_last_character) output.push($char);
			...
			if(state == error_invalid_placeholder) {
				if(!safe) throw "Invalid placeholder in string: line " + Std.string(lineno) + ", col " + Std.string(col); else {
					output.push(this.template.substring(point_delimiter,i + 1));
					state = state_not_interpolation;
					point_delimiter = point_left_bracket = point_right_bracket = -1;
					point_variable_start = point_variable_end = -1;
				}
	...
	,log: function(message) {
		if(interpolation.Template.debug) console.log(message);
	}
	,is_id: function($char) {
		var char_code = HxOverrides.cca($char,0);
		if(char_code >= 65 && char_code <= 90 || char_code >= 97 && char_code <= 122 || char_code >= 48 && char_code <= 57 || $char == "-" || $char == "_") return true; else return false;
	}
	,__class__: interpolation.Template
}
var js = {}
js.Boot = function() { }
...
var logging = {}
logging.ArrayHelper = function() { }
$hxClasses["logging.ArrayHelper"] = logging.ArrayHelper;
logging.ArrayHelper.__name__ = ["logging","ArrayHelper"];
logging.ArrayHelper.has = function(array,object) {
	var found = false;
	var _g = 0;
...
logging.IFilterer = function() { }
$hxClasses["logging.IFilterer"] = logging.IFilterer;
logging.IFilterer.__name__ = ["logging","IFilterer"];
logging.IFilterer.prototype = {
	__class__: logging.IFilterer
}
logging.Filterer = function() {
	this.filters = [];
};
$hxClasses["logging.Filterer"] = logging.Filterer;
logging.Filterer.__name__ = ["logging","Filterer"];
logging.Filterer.__interfaces__ = [logging.IFilterer];
logging.Filterer.prototype = {
	filter: function(record) {
		var enable = true;
		var _g = 0, _g1 = this.get_filters();
		while(_g < _g1.length) {
			var f = _g1[_g];
			++_g;
	...
	,removeFilter: function(filter) {
		HxOverrides.remove(this.get_filters(),filter);
	}
	,addFilter: function(filter) {
		...
		while(_g < _g1.length) {
			var f = _g1[_g];
			++_g;
			if(f == filter) found = true;
		}
		if(!found) this.get_filters().push(filter);
	}
	,get_filters: function() {
		return this.filters;
	}
	,__class__: logging.Filterer
	,__properties__: {get_filters:"get_filters"}
}
logging.IFormatter = function() { }
$hxClasses["logging.IFormatter"] = logging.IFormatter;
logging.IFormatter.__name__ = ["logging","IFormatter"];
logging.IFormatter.prototype = {
	__class__: logging.IFormatter
}
logging.Formatter = function(fmt,datefmt) {
	if(null == fmt) fmt = "$levelname $name $message";
	if(null == datefmt) datefmt = "YYYY-MM-DD hh:mm:ss";
	this.fmt = fmt;
	this.datefmt = datefmt;
	this.template = new interpolation.Template(this.fmt);
};
$hxClasses["logging.Formatter"] = logging.Formatter;
logging.Formatter.__name__ = ["logging","Formatter"];
logging.Formatter.__interfaces__ = [logging.IFormatter];
logging.Formatter.prototype = {
	format: function(record) {
		if(this.usesTime()) record.asctime = this.formatTime(record);
		var f = this.template.safe_substitute(record.toStringMap());
		if(record.stack != null) {
			if(record.stackText == null) record.stackText = this.formatException(record.stack);
			f += record.stackText;
		}
		return f;
	}
	,usesTime: function() {
		if(this.fmt.indexOf("$asctime") > -1) return true; else return false;
	}
	,formatTime: function(record) {
		var textual = HxOverrides.dateStr((function($this) {
			var $r;
			var d = new Date();
			d.setTime(record.created);
			$r = d;
			return $r;
		}(this)));
		return textual + "," + Std.string(record.millisecond);
	}
	,formatException: function(stack) {
		return haxe.CallStack.toString(stack);
	}
	,getFMT: function() {
		return this.fmt;
	}
	,__class__: logging.Formatter
}
logging.IHandler = function() { }
$hxClasses["logging.IHandler"] = logging.IHandler;
logging.IHandler.__name__ = ["logging","IHandler"];
logging.IHandler.__interfaces__ = [logging.IFilterer];
logging.IHandler.prototype = {
	__class__: logging.IHandler
}
logging.Handler = function(level) {
	if(level == null) level = 0;
	this.level = level;
	this.formatter = new logging.Formatter();
	this.filterer = new logging.Filterer();
};
$hxClasses["logging.Handler"] = logging.Handler;
logging.Handler.__name__ = ["logging","Handler"];
logging.Handler.__interfaces__ = [logging.IHandler];
logging.Handler.prototype = {
	get_filters: function() {
		return this.filterer.get_filters();
	}
	,emit: function(record) {
		throw "Not implement.";
	}
	,handle: function(record) {
		var available = this.filter(record);
		if(available) this.emit(record);
		return available;
	}
	,format: function(record) {
	...
	,filter: function(record) {
		return this.filterer.filter(record);
	}
	,removeFilter: function(filter) {
		this.filterer.removeFilter(filter);
	}
	,addFilter: function(filter) {
		this.filterer.addFilter(filter);
	}
	,__class__: logging.Handler
	,__properties__: {get_filters:"get_filters"}
}
logging.IBaseLogger = function() { }
$hxClasses["logging.IBaseLogger"] = logging.IBaseLogger;
logging.IBaseLogger.__name__ = ["logging","IBaseLogger"];
logging.IFilter = function() { }
$hxClasses["logging.IFilter"] = logging.IFilter;
logging.IFilter.__name__ = ["logging","IFilter"];
logging.IFilter.prototype = {
	__class__: logging.IFilter
}
logging.ILogger = function() { }
$hxClasses["logging.ILogger"] = logging.ILogger;
logging.ILogger.__name__ = ["logging","ILogger"];
logging.ILogger.__interfaces__ = [logging.IFilterer,logging.IBaseLogger];
logging.ILogger.prototype = {
	__class__: logging.ILogger
}
logging.ILoggerFactory = function() { }
$hxClasses["logging.ILoggerFactory"] = logging.ILoggerFactory;
logging.ILoggerFactory.__name__ = ["logging","ILoggerFactory"];
logging.ILoggerFactory.prototype = {
	__class__: logging.ILoggerFactory
}
logging.IPlaceHolder = function() { }
$hxClasses["logging.IPlaceHolder"] = logging.IPlaceHolder;
logging.IPlaceHolder.__name__ = ["logging","IPlaceHolder"];
logging.IPlaceHolder.__interfaces__ = [logging.IBaseLogger];
logging.IPlaceHolder.prototype = {
	__class__: logging.IPlaceHolder
}
logging.Level = function() { }
$hxClasses["logging.Level"] = logging.Level;
logging.Level.__name__ = ["logging","Level"];
logging.Level.getName = function(level) {
	if(null == logging.Level.levelNames) {
		logging.Level.levelNames = new haxe.ds.IntMap();
		logging.Level.levelNames.set(logging.Level.CRITICAL,"CRITICAL");
		logging.Level.levelNames.set(logging.Level.ERROR,"ERROR");
		logging.Level.levelNames.set(logging.Level.WARNING,"WARNING");
		logging.Level.levelNames.set(logging.Level.INFO,"INFO");
		logging.Level.levelNames.set(logging.Level.DEBUG,"DEBUG");
		logging.Level.levelNames.set(logging.Level.NOTSET,"NOTSET");
	}
	return logging.Level.levelNames.get(level);
}
logging.LogRecord = function(name,level,pathname,lineno,message,$arguments,stack) {
	this.name = name;
	this.level = level;
	this.levelname = logging.Level.getName(level);
	this.levelno = level;
	this.pathname = pathname;
	this.lineno = lineno;
	this.created = new Date().getTime() / 1000 | 0;
	this.millisecond = (haxe.Timer.stamp() - this.created) * 1000 | 0;
	var _arguments;
	if(null == $arguments) this.message = message; else {
		var template = new interpolation.Template(message);
		var _g = Type["typeof"]($arguments);
		var $e = (_g);
		switch( $e[1] ) {
		case 4:
			_arguments = new haxe.ds.StringMap();
			var _g1 = 0, _g2 = Reflect.fields($arguments);
			while(_g1 < _g2.length) {
				var key = _g2[_g1];
				++_g1;
		...
		case 6:
			var c = $e[2];
			if(!js.Boot.__instanceof($arguments,haxe.ds.StringMap)) throw "Argument type requirement StringMap";
			break;
		default:
...
$hxClasses["logging.LogRecord"] = logging.LogRecord;
logging.LogRecord.__name__ = ["logging","LogRecord"];
logging.LogRecord.prototype = {
	toString: function() {
		return "";
	}
	,toStringMap: function() {
		var context = this.cloneStringMap(this["arguments"]);
		var keys = ["name","level","levelno","levelname","pathname","lineno","message","arguments","asctime","created","millisecond"];
		var _g = 0;
	...
	,cloneStringMap: function(hash) {
		var cloned = new haxe.ds.StringMap();
		if(null != hash) {
			var $it0 = hash.keys();
			while( $it0.hasNext() ) {
				var key = $it0.next();
...
$hxClasses["logging.Logger"] = logging.Logger;
logging.Logger.__name__ = ["logging","Logger"];
logging.Logger.__interfaces__ = [logging.ILogger];
logging.Logger.prototype = {
	get_filters: function() {
		return this.filterer.get_filters();
	}
	,notEqual: function(logger) {
		return !this.equal(logger);
	}
	,equal: function(logger) {
		return true;
	}
	,getChild: function(suffix) {
		if(logging.Logger.root != this) suffix = [this.name,suffix].join(".");
		return this.manager.getLogger(suffix);
	}
	,callHandlers: function(record) {
		var _logger = this;
		var found = 0;
		while(null != _logger) {
			var _g = 0, _g1 = _logger.handlers;
			while(_g < _g1.length) {
				var handler = _g1[_g];
				++_g;
				found += 1;
				if(record.levelno >= handler.level) handler.handle(record);
			}
			if(!_logger.propagate) _logger = null; else _logger = _logger.parent;
		}
		if(0 == found && !this.manager.emittedNoHandlerWarning) this.manager.emittedNoHandlerWarning = true;
	}
	,handle: function(record) {
		if(logging.Logger.globalHandler != null) logging.Logger.globalHandler.handle(record);
		if(!this.disabled && this.filterer.filter(record)) this.callHandlers(record);
	}
	,_log: function(level,message,$arguments,stack,pos) {
		var record = new logging.LogRecord(this.name,level,pos.fileName,pos.lineNumber,message,$arguments,stack);
		this.handle(record);
	}
	,log: function(level,message,$arguments,stack,pos) {
		if(this.isEnableFor(level)) this._log(level,message,$arguments,stack,pos);
	}
	,fatal: function(message,$arguments,stack,pos) {
		if(this.isEnableFor(logging.Level.CRITICAL)) this._log(logging.Level.CRITICAL,message,$arguments,stack,pos);
	}
	,critical: function(message,$arguments,stack,pos) {
		if(this.isEnableFor(logging.Level.CRITICAL)) this._log(logging.Level.CRITICAL,message,$arguments,stack,pos);
	}
	,error: function(message,$arguments,stack,pos) {
		if(this.isEnableFor(logging.Level.ERROR)) this._log(logging.Level.ERROR,message,$arguments,stack,pos);
	}
	,warn: function(message,$arguments,stack,pos) {
		if(this.isEnableFor(logging.Level.WARNING)) this._log(logging.Level.WARNING,message,$arguments,stack,pos);
	}
	,warning: function(message,$arguments,stack,pos) {
		if(this.isEnableFor(logging.Level.WARNING)) this._log(logging.Level.WARNING,message,$arguments,stack,pos);
	}
	,info: function(message,$arguments,stack,pos) {
		if(this.isEnableFor(logging.Level.INFO)) this._log(logging.Level.INFO,message,$arguments,stack,pos);
	}
	,debug: function(message,$arguments,stack,pos) {
		if(this.isEnableFor(logging.Level.DEBUG)) this._log(logging.Level.DEBUG,message,$arguments,stack,pos);
	}
	,isEnableFor: function(level) {
		if(this.manager.disable >= level) return false;
		return level >= this.getEffectiveLevel();
	}
	,getEffectiveLevel: function() {
		var logger = this;
		while(true) {
			if(null == logger) break;
	...
	,addHandler: function(handler) {
		if(!logging.ArrayHelper.has(this.handlers,handler)) this.handlers.push(handler);
	}
	,filter: function(record) {
		return this.filterer.filter(record);
	}
	...
	,__class__: logging.Logger
	,__properties__: {get_filters:"get_filters"}
}
logging.LoggerFactory = function() {
};
$hxClasses["logging.LoggerFactory"] = logging.LoggerFactory;
logging.LoggerFactory.__name__ = ["logging","LoggerFactory"];
logging.LoggerFactory.__interfaces__ = [logging.ILoggerFactory];
logging.LoggerFactory.shared = function() {
	if(null == logging.LoggerFactory._shared) logging.LoggerFactory._shared = new logging.LoggerFactory();
	return logging.LoggerFactory._shared;
}
logging.LoggerFactory.prototype = {
	create: function(name,level) {
		if(level == null) level = 0;
		return new logging.Logger(name,level);
	}
	,__class__: logging.LoggerFactory
}
logging.RootLogger = function(level) {
	logging.Logger.call(this,"root",level);
};
$hxClasses["logging.RootLogger"] = logging.RootLogger;
logging.RootLogger.__name__ = ["logging","RootLogger"];
logging.RootLogger.__super__ = logging.Logger;
logging.RootLogger.prototype = $extend(logging.Logger.prototype,{
	__class__: logging.RootLogger
});
logging.Manager = function(root) {
	this.root = root;
	this.disable = 0;
	this.loggerMap = new haxe.ds.StringMap();
	this.loggerFactory = logging.LoggerFactory.shared();
	this.emittedNoHandlerWarning = false;
};
$hxClasses["logging.Manager"] = logging.Manager;
logging.Manager.__name__ = ["logging","Manager"];
logging.Manager.prototype = {
	_fixupChildren: function(placeholder,alogger) {
		var name = alogger.name;
		var name_size = name.length;
		var $it0 = placeholder.loggerMap.iterator();
		while( $it0.hasNext() ) {
			var logger = $it0.next();
	...
	,_fixupParents: function(alogger) {
		var name = alogger.name;
		var parent_name;
		...
		while(i > 0 && null == rv) {
			...
			if(!this.loggerMap.exists(parent_name)) this.loggerMap.set(parent_name,new logging.PlaceHolder(alogger)); else {
				obj = this.loggerMap.get(parent_name);
				if(js.Boot.__instanceof(obj,logging.ILogger)) rv = obj; else if(js.Boot.__instanceof(obj,logging.IPlaceHolder)) (js.Boot.__cast(obj , logging.IPlaceHolder)).append(alogger);
			}
			i = parent_name.lastIndexOf(".");
		}
		if(null == rv) rv = this.root;
		alogger.parent = rv;
	}
	,getLogger: function(name) {
		...
		var baseLogger;
		var placeholder;
		if(this.loggerMap.exists(name)) {
			baseLogger = this.loggerMap.get(name);
			if(js.Boot.__instanceof(logger,logging.IPlaceHolder)) {
				placeholder = baseLogger;
				logger = this.loggerFactory.create(name);
				logger.manager = this;
				this.loggerMap.set(name,logger);
				this._fixupChildren(placeholder,logger);
				this._fixupParents(logger);
			}
			logger = baseLogger;
		} else {
			logger = this.loggerFactory.create(name);
	...
	,__class__: logging.Manager
}
logging.handlers = {}
logging.handlers.StreamHandler = function(stream) {
	logging.Handler.call(this);
	this.name = "StreamHandler";
	this.stream = stream;
};
$hxClasses["logging.handlers.StreamHandler"] = logging.handlers.StreamHandler;
logging.handlers.StreamHandler.__name__ = ["logging","handlers","StreamHandler"];
logging.handlers.StreamHandler.__interfaces__ = [logging.IHandler];
logging.handlers.StreamHandler.__super__ = logging.Handler;
logging.handlers.StreamHandler.prototype = $extend(logging.Handler.prototype,{
	emit: function(record) {
		var message;
		try {
			...
			console.log("Stream is null " + Std.string(null == this.stream));
			console.log(e);
			console.log(haxe.CallStack.toString(haxe.CallStack.exceptionStack()));
		}
	}
	,flush: function() {
		this.stream.flush();
	}
	,__class__: logging.handlers.StreamHandler
});
logging.Logging = function() { }
$hxClasses["logging.Logging"] = logging.Logging;
logging.Logging.__name__ = ["logging","Logging"];
logging.Logging.info = function(message,$arguments,stack,pos) {
	logging.Logging.getLogger(null).info(message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 21, className : "logging.Logging", methodName : "info"});
}
logging.Logging.debug = function(message,$arguments,stack,pos) {
	logging.Logging.getLogger(null).debug(message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 27, className : "logging.Logging", methodName : "debug"});
}
logging.Logging.warning = function(message,$arguments,stack,pos) {
	logging.Logging.getLogger(null).warning(message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 33, className : "logging.Logging", methodName : "warning"});
}
logging.Logging.warn = function(message,$arguments,stack,pos) {
	logging.Logging.getLogger(null).warn(message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 39, className : "logging.Logging", methodName : "warn"});
}
logging.Logging.error = function(message,$arguments,stack,pos) {
	logging.Logging.getLogger(null).error(message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 45, className : "logging.Logging", methodName : "error"});
}
logging.Logging.critical = function(message,$arguments,stack,pos) {
	logging.Logging.getLogger(null).critical(message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 51, className : "logging.Logging", methodName : "critical"});
}
logging.Logging.fatal = function(message,$arguments,stack,pos) {
	logging.Logging.getLogger(null).fatal(message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 57, className : "logging.Logging", methodName : "fatal"});
}
logging.Logging.log = function(level,message,stack,$arguments,pos) {
	logging.Logging.getLogger(null).log(level,message,$arguments,null,{ fileName : "Logging.hx", lineNumber : 64, className : "logging.Logging", methodName : "log"});
}
logging.Logging.getLogger = function(name) {
	if(null == logging.Logger.root) logging.Logger.root = new logging.RootLogger(0);
	if(null == logging.Logging.manager) {
		logging.Logging.manager = new logging.Manager(logging.Logger.root);
		logging.Logger.root.manager = logging.Logging.manager;
	}
	var logger;
	if(null == name || "" == name) logger = logging.Logging.manager.root; else logger = logging.Logging.manager.getLogger(name);
	return logger;
}
logging.PlaceHolder = function(alogger) {
	this.loggerMap = new haxe.ds.StringMap();
	this.loggerMap.set(alogger.name,alogger);
};
$hxClasses["logging.PlaceHolder"] = logging.PlaceHolder;
logging.PlaceHolder.__name__ = ["logging","PlaceHolder"];
logging.PlaceHolder.__interfaces__ = [logging.IPlaceHolder];
logging.PlaceHolder.prototype = {
	append: function(alogger) {
		this.loggerMap.set(alogger.name,alogger);
	}
	,__class__: logging.PlaceHolder
}
var openfl = {}
openfl.display = {}
...
var logger = logging.Logging.getLogger("");
